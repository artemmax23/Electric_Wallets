...
@pytest.fixture
async def created_wallet(client: AsyncClient):
    """Создаёт кошелёк и возвращает его ID и баланс."""
    response = await client.post("/api/v1/wallets/", json={"balance": 1000})
    assert response.status_code == 201
    data = response.json()
    return {"id": data["id"], "balance": data["balance"]}
    
@pytest.fixture
async def empty_wallet(client: AsyncClient):
    """Создаёт пустой кошелёк (баланс 0)."""
    response = await client.post("/api/v1/wallets/", json={"balance": 0})
    assert response.status_code == 201
